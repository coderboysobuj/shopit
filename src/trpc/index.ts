...
import { categorySchema, productSchema, signUpSchema } from '@/validator/schema';
...
export const appRouter = router({
  signUp: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (userExists) {
        throw new TRPCError({ message: "Email address already exists", code: 'BAD_REQUEST' })
      }
  ...
  categoryStore: privateProcedure.input(categorySchema).mutation(async ({ input, ctx }) => {
  ...
  productStore: privateProcedure.input(productSchema).mutation(async ({ input, ctx }) => {
