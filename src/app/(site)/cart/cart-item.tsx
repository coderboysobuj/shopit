...
export default function CartItem({ item, setProductQuantity }: CartITemProps) {
  ...
  return (
    <li className="flex gap-4 items-center">
      ...
      <div className="flex w-full flex-col flex-auto  items-center gap-4 md:flex-row ">
        ...
        <h4 className='text-md text-muted-foreground'>${product.price.toFixed(2)}</h4>

        {isPending ? (
          <Loader2 className='animate-spin h-5 w-5' />
        ) : (
          <select className='px-2 py-2 rounded-md bg-muted' defaultValue={quantity.toString()} onChange={(e) => {
